...
def classify_face():
    ...
    while True:
        ...
        if process_this_frame:
            ...
            # VIP
            if (time.time() - start_time > period_time) and sameface and sameface_name != "unknown" and sameface_name: #and sameface_name not in vip_met:
                index = names_idx.index(sameface_name)
                #vip_met.append(sameface_name)
                ...
                mixer.init()
                mixer.music.load(
                    "assets/NOAH - Voice for TBKK/TBKK - Thai 35 Users/"+voice_list[index])
                mixer.music.play()
